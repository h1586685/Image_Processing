...
transformation = tk.Menu(image_edit, tearoff=False)
rotation_menu = tk.Menu(image_edit,tearoff=False)
rotation_menu.add_command(label="向左旋轉90度",command = lambda: rotation_img(window,get_img(),90))
rotation_menu.add_command(label="向右旋轉90度",command = lambda: rotation_img(window,get_img(),-90))
rotation_menu.add_command(label="垂直翻轉",command = lambda: flip_img(window,get_img(),0))
rotation_menu.add_command(label="水平翻轉",command = lambda: flip_img(window,get_img(),1))
transformation.add_cascade(label="旋轉",menu=rotation_menu)

transformation.add_command(label="平移",command = lambda: translate_img_window(window))
transformation.add_command(label="仿射轉換",command= lambda: affine_trans_win(window))
transformation.add_command(label="透視",command= lambda:perspective_trans(window))
image_edit.add_cascade(label="變形" ,menu=transformation)
